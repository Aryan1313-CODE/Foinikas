...
const filePath = "/Deep_View_returns.csv";
const SMOOTHING_WINDOW = 30;

const DeepViewChart = ({ onStatsChange }) => {
      ...
      complete: (result) => {
        const cleaned = result.data
          .map((row) => {
            ...
            const date = new Date(rawDate?.replace(/-/g, "/")); // Safari safe
  ...
  return (
    <Plot
      revision={smoothData.length} // iOS re-render stability
      data={[
        {
      ...
      layout={{
        ...
        title: {
          text: isMobile
            ? "Deep View -<br>Historical Portfolio Performance"
            : "Deep View - Historical Portfolio Performance",
          font: {
            color: "#800000",
            size: isMobile ? 14 : 18,
          },
